...
static int do_load(int fd, symtab_t symtab)
{
    ...
    for (i = 0, p = shdr; i < ehdr.e_shnum; i++, p++)
        if (SHT_SYMTAB == p->sh_type) {
            if (symh) {
                HOOKLOG("too many symbol tables", 0);
                goto out;
            }
            symh = p;
            ...
            if (strh) {
                HOOKLOG("too many string tables", 0);
                goto out;
            }
            strh = p;
        } else if (SHT_STRTAB == p->sh_type
    ...
    if ((!dynsymh && dynstrh) || (dynsymh && !dynstrh)) {
        HOOKLOG("bad dynamic symbol table", 0);
        goto out;
    }
    if ((!symh && strh) || (symh && !strh)) {
        HOOKLOG("bad symbol table", 0);
        goto out;
    }
    if (!dynsymh && !symh) {
        HOOKLOG("no symbol table", 0);
        goto out;
    }
...
int find_name(pid_t pid, char *name, char *libn, unsigned long *addr)
{
    ...
    if (0 > load_memmap(pid, mm, &nmm)) {
        HOOKLOG("cannot read memory map", 0);
        return -1;
    }
    if (0 > find_libname(libn, libc, sizeof(libc), &libcaddr, mm, nmm)) {
        HOOKLOG("cannot find lib: %s", libn);
        return -1;
    }
    //HOOKLOG("lib: >%s<\n", libc)
    s = load_symtab(libc);
    if (!s) {
        HOOKLOG("cannot read symbol table",0);
        return -1;
    }
    if (0 > lookup_func_sym(s, name, addr)) {
        HOOKLOG("cannot find function: %s", name);
        return -1;
    }
    *addr += libcaddr;
    return 0;
